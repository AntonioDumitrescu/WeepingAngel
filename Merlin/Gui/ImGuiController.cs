...
namespace Merlin.Gui
{
    ...
    public class ImGuiController : IDisposable
    {
        ...
        public unsafe ImGuiController(GraphicsDevice gd, Sdl2Window window, OutputDescription outputDescription, int width, int height)
        {
            ...
            var mainViewport = platformIO.Viewports[0];
            ...
            io.NativePtr->BackendPlatformName = (byte*)new NullTerminatedString("Veldrid.SDL2 Backend").DataPtr;
        ...
        {
            var window = new VeldridImGuiWindow(_gd, vp);
        }
        ...
        private byte[] LoadEmbeddedShaderCode(ResourceFactory factory, string name, ShaderStages stage)
        {
            switch (factory.BackendType)
            {
                ...
                default:
                    throw new NotImplementedException($"Backend {factory.BackendType} not supported.");
            }
        }
            ...
            if (_frameBegun)
            {
                ...
                if ((ImGuiNET.ImGui.GetIO().ConfigFlags & ImGuiConfigFlags.ViewportsEnable) != 0)
                {
                    ...
                    for (int i = 1; i < platformIo.Viewports.Size; i++)
                    {
                        ...
                        var window = (VeldridImGuiWindow)GCHandle.FromIntPtr(vp.PlatformUserData).Target;
        ...
        private unsafe void UpdateMonitors()
        {
            ...
            var numMonitors = p_sdl_GetNumVideoDisplays();
        ...
        private void UpdateImGuiInput(InputSnapshot snapshot)
        {
            ...
            unsafe
            {
                var buttons = p_sdl_GetGlobalMouseState(&x, &y);
            ...
            var viewports = ImGuiNET.ImGui.GetPlatformIO().Viewports;
            for (var i = 1; i < viewports.Size; i++)
            {
                var v = viewports[i];
                var window = ((VeldridImGuiWindow)GCHandle.FromIntPtr(v.PlatformUserData).Target);
                window.Update();
            }
        }
